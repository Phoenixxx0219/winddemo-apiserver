...
def batch_process(date, algorithm, size, reflectivity_threshold, poolingScale, 
                  datatype=12, path="/data/MaxPool", interval_minutes=6):
    if poolingScale == 0:
...
def monomer_tracking(date, algorithm, size=1000, reflectivity_threshold=20, poolingScale=0, 
                     interval_minutes=6, lookup_table_path="/data/lookup_table.npy"):
    """
    date: 时间
    algorithm (int): 预测算法。
...
if __name__=='__main__':
    ...
    with open("./out/out1.json", 'w', encoding='utf-8') as f:
        ...
        for entity in entities:
            ...
            if span_data_list:
                for data in span_data_list:
                    ...
                    u = data.get('u')
                    v = data.get('v')
                    print("time:", time, ", direction:", direction, ", x:", x, ", y:", y, ", u:", u, ", v:", v)
            else:
                print(f"Entity with id {entity.get('id')} has no spanData.")


    check_start_time_equality("./out/out1.json")